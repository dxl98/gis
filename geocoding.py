...
def baidu_gecoding(placename, baidu_api_key):
...
def banfrance_gecoding(placename):
...
def bing_gecoding(placename, bing_api_key):
...
def canadapost_gecoding(placename, canadapost_api_key):
...
def geocodeearth_gecoding(placename, geocodeearth_api_key):
...
def geocodefarm_gecoding(placename, geocodefarm_api_key):
...
def gaode_gecoding(placename, gaode_api_key):
...
def geolake_gecoding(placename, geolake_api_key):
...
def geonames_gecoding(placename, geonames_api_key):
...
def geoottawa_gecoding(placename):
...
def gisgraphy_gecoding(placename):
...
def here_gecoding(placename, here_api_key):
...
def ignfrance_gecoding(placename):
...
def locationiq_gecoding(placename, locationiq_api_key):
...
def mapbox_gecoding(placename, mapbox_api_key):
...
def mapquest_gecoding(placename, mapquest_api_key):
...
def maptiler_gecoding(placename, maptiler_api_key):
...
def opencage_gecoding(placename, opencage_api_key):
...
def openstreetmap_gecoding(placename):
...
def pickpoint_gecoding(placename, pickpoint_api_key):
...
def uscensus_gecoding(placename):
...
def what3words_gecoding(placename, what3words_api_key):
    what3words_geolocator = What3WordsV3(api_key=what3words_api_key)
    what3words_location = what3words_geolocator.geocode(placename)
    print("\nWhat3WordsV3 API:")
    what3words_DD1 = (what3words_location.latitude, what3words_location.longitude)
    what3words_CRS = "World Geodetic System 1984 (WGS84)"
    print(what3words_location.latitude, what3words_location.longitude)
    print("CRS：World Geodetic System 1984 (WGS84)")
    return what3words_DD1, what3words_CRS


def yahoo_gecoding(placename):
